...
* 
*/
class Ion_custom_model extends MY_Model {
	 ...
	 */
	public function users_meta($what = NULL){
		$this->trigger_events('users_meta');
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users_meta'].'.*',
			    $this->tables['users_meta'].'.id as id',
			    $this->tables['users_meta'].'.user_id as user_id'
			));
		}

		$this->db->from($this->tables['users_meta']);
	...
	public function sort_meta($meta = array()){
		if(empty($meta)){
			return false;
		}
		...
		foreach ($meta as $key => $value) {
			$k = str_replace(' ', '_', $value['name']);
			$sorted[$k] = $value['value'];
		}
	 ...
	 * @author Pablo Villalba
	 */
	public function user_meta($id = NULL){
		$this->trigger_events('user_meta');

		// if no id was passed use the current users id
		$id = isset($id) ? $id : $this->session->userdata('user_id');

		$this->order_by($this->tables['users_meta'].'.id', 'asc');
		$this->where($this->tables['users_meta'].'.user_id', $id);

		$this->users_meta(array('name', 'value'));
	 ...
	 **/
	public function update_meta($id, $data = array(), $skip_validation = false)
	{
		$this->trigger_events('pre_update_user_meta');
			...
			$user->{$this->identity_column} !== $data[$this->identity_column])
		{
			...
			$this->trigger_events(array('post_update_user_meta', 'post_update_user_meta_unsuccessful'));
			$this->set_error('update_unsuccessful');
			return FALSE;
		}

		$this->trigger_events('extra_where');
		//$this->db->update($this->tables['users_meta'], $data, array('user_id' => $user->id));
		$data = $this->_filter_meta_data($this->tables['users_meta'], $data);

		foreach ($data as $key => $value) {

			$this->db->where('name', $key);
			$query = $this->db->get($this->tables['users_meta']);
			if($query->num_rows() > 0){
				$this->db->where('user_id', $id)->like('name', $key);
				$this->db->update($this->tables['users_meta'], array('value'=>$value));
			}else{
				$insert = array(
					'user_id'=>$id,
					'name'=>$key,
				...
				$this->db->insert($this->tables['users_meta'], $insert);
			}
	...
	public function add_insulin($id, $insulins = array()){
		if(empty($insulins)){
			return false;
		}

		$this->trigger_events('pre_add_insulin');

		$user = $this->user($id)->row();

		$this->db->trans_begin();

		foreach ((array)$insulins as $insulin) {

			$this->db->where('id_user', $user->id)->where('id_med', $insulin);
			$query = $this->db->get($this->tables['user_meds']);

			if($query->num_rows() > 0){
				$this->db->where('id_user', $user->id)->where('id_med', $insulin);
				$this->db->update($this->tables['user_meds'], array('current'=>1));
				$this->trigger_events(array('post_reactivate_insulin', 'post_update_user_successful'));
				$this->set_message('update_successful');
			}else{
				$insert = array(
					'id_user'=>$id,
				...
				$this->db->insert($this->tables['user_meds'], $insert);
				$this->trigger_events(array('post_add_insulin', 'post_update_user_successful'));
			}
		...
		if ($this->db->trans_status() === FALSE){
			...
			$this->trigger_events(array('post_add_insulin', 'post_add_insulin_unsuccessful'));
			$this->set_error('update_unsuccessful');
			return FALSE;
		}
		...
		$this->trigger_events(array('post_add_insulin', 'post_add_insulin_successful'));
		$this->set_message('update_successful');
		return TRUE;

	}

	public function delete_insulin($id, $insulin){
		...
		//$this->db->start_cache();
		$this->db->where('id_user', $user->id)->where('id_med', $insulin)->where('current', 1);
		//$this->db->stop_cache();
		$query = $this->db->get($this->tables['user_meds']);
		//$this->db->flush_cache();
		...
		if($query->num_rows() > 0){
			$this->db->where('id_user', $user->id)->where('id_med', $insulin);
			$new = array('current'=>0);
			$this->db->update($this->tables['user_meds'], $new);
			$this->trigger_events(array('post_deactivate_insulin', 'post_update_user_successful'));
			$this->set_message('update_successful');
			return TRUE;
		}
		...
		if ($this->db->trans_status() === FALSE){
			...
			$this->trigger_events(array('post_delete_insulin', 'post_delete_insulin_unsuccessful'));
			$this->set_error('update_unsuccessful');
			return FALSE;
		}
		...
		$this->trigger_events(array('post_delete_insulin', 'post_delete_insulin_successful'));
		$this->set_message('update_successful');
		return TRUE;
	}

	public function _filter_meta_data($table, $data){

		// Messure
		if(isset($data['messure'])){
			if($data['messure'] == 'false'){
				$data['messure'] = false;
			}
			$data['messure'] = ($data['messure'] == true) ? 'Mmol/l' : 'Mg/dl';
		}
	...
	/*
	 * Retrieve the user->row() function from the main model and 
	 * some extra data added for the specific app
	 */
	public function user_full(){
		$user = $this->user()->row();
		$meta = $this->sort_meta($this->user_meta()->result_array());
...
class Ion_auth_model extends Ion_custom_model
{
	/**
